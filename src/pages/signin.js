...
const SignIn = ({ location }) => {
  ...
  const login = function (e) {
    e.preventDefault();
    auth.signIn(formData.email, formData.password).then((res) => {
      if (res.succeeded === false) {
        if (res.error.code === 'auth/wrong-password') {
          setError({
            isError: true,
            msg: `${t`Username or password incorrect. Did you sign up with`} PBC Access?`,
          });
        } else {
    ...
    <Layout location={location}>
      ...
      >
        <div className="container py-5">
          <h1
            className="kapra font-weight-bold my-4 display-1 text-center"
            style={{ color: '#e8e1cf' }}
          >{t`Sign In To Your Account`}</h1>
        </div>
      </section>
      <section className="container reduced-width my-5">
        ...
        <form onSubmit={login}>
          ...
          >{t`Sign In`}</button>
          <Link
            to={translateLink(`/forgot-password`, language)}
            state={{ from: location.state?.from || '/signin' }}
            class="btn btn-link btn-sm text-primary mt-3"
          >{t`Forgot your password?`}</Link>
          <Link
            to={translateLink(`/signup${queryString}`, language)}
            state={{
              from: location.state?.from || translateLink('/signin', language),
            }}
            class="btn btn-link btn-sm text-primary mt-3"
          >{t`Create an account`}</Link>
        </form>
        {canUsePbcSSO && (
          <div class="mhr-signin">
