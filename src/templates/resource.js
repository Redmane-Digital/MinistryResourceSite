...
const Video = ({ resource, isAuthenticated, downloads, loc, saveResource }) => {
  const embedUrl = resource.vimeoUrl?.replace(
    'https://vimeo.com/',
    ...
    <>
      <VideoSection className="py-5">
        <div className="container reduced-width text-tan">
          <p
            className="mb-2 text-tan cursor-pointer"
          ...
          {isAuthenticated ? (
            ...
            <div
              ...
              style={{
                backgroundImage: `linear-gradient(rgba(0,0,0,0.8),rgba(0,0,0,0.8)), url(${resource?.thumbnail?.url})`,
          ...
          <div className="col-12">
            <div className="mt-3">
              {resource.tags.map((tag) => (
                <Tag
                  className="badge rounded-pill bg-light text-dark mr-3 mb-3"
                  key={tag.title[language]}
                >
                  {tag.title[language]}
                </Tag>
              ))}
            ...
            <div className="mt-5 pt-4">
              ...
              <hr />
              {isAuthenticated ? (
                resource.downloads.map((download, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        className="text-dark ml-4 pl-1 ml-sm-0 pl-sm-0 pl-md-1 ml-md-1 pl-lg-0 ml-lg-0 d-inline-block"
                        data-filename={fileName}
    ...
    <section className="container reduced-width my-5">
      <div className="row gx-5">
        <div className="col-12 col-md-5">
          <img
            src={resource.thumbnail ? resource.thumbnail.url : placeholder}
            className="w-100"
              ...
              resource.downloads.map((download, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className="text-dark ml-4 pl-1 ml-sm-0 pl-sm-0 pl-md-1 ml-md-1 pl-lg-0 ml-lg-0 d-inline-block"
                      style={{ opacity: 0.4, cursor: 'pointer', minWidth: 101 }}
                      data-filename={fileName}
                    >
                      {t`Download`}{' '}
                      <FontAwesomeIcon
...
const Resource = ({ location, pageContext }) => {
  ...
  return (
    <Layout location={location}>
      <SEO
        title={`${resource.title[language]} | ${t`Resources`}`}
        ...
        <div className="container reduced-width py-3">
          <h4 className="text-white mb-0">
            ...
            {resource.categories.length ? (
              <Link
                to={`/resources/${resource.categories[0].slug}`}
                className="text-white"
              >
                {resource.categories[0].title[language]}
              </Link>
            ) : (
              ''
            )}
