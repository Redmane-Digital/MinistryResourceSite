...
import ContentTypeIcon from '../../atoms/ContentTypeIcon';
...
const LiveSearchBox = (props) => {
  ...
  useEffect(() => {
    const toggle = () => {
      if (results.current && results.current.classList) {
        if (
          query.length === 0 &&
          results.current.classList.contains('searching')
        )
          hide();
        else if (
          query.length > 0 &&
  ...
  return (
    <SearchBox className={`container reduced-width py-3 ${width}`}>
      {!noTitle && (
        <h2
          className={`text-center font-weight-${fontWeight} text-${textColor} ${mt} pb-3`}
        >{t`Start Searching for the Resource You Need`}</h2>
      )}
      <div className="box-shadow mt-4 mb-2 bg-white">
        <form className="input-group input-group-lg" onSubmit={Search}>
          ...
          <input
            ...
            placeholder={t`E.g. Worship Job Description`}
            aria-label="search"
          ...
          />
          <ClearSearch
            className={`input-group-text ${query.length < 1 ? 'd-none' : ''}`}
            type="button"
            id="addon-wrapping"
            onClick={() => setQuery('')}
          >
            ...
            {state.isSearching ? (
              ...
              <>
                <div className="d-flex justify-content-between align-items-center px-5 px-md-3 mb-4">
                  ...
                  {state.resourcesResults.length > 0 && (
                    <Link
                      className="d-block text-right mb-0 font-weight-bold small"
                      to={translateLink(
                ...
                {state.resourcesResults.length === 0 && (
                  <h5 class="px-3">
                    <em>{t`No results to show`}</em>
                  </h5>
                )}
                {state.resourcesResults.slice(0, 2).map((result) => {
                  return (
                    <Link
                      to={`/resource/${result.slug}`}
                      className="row gx-5 px-5 px-md-3"
                      key={result.slug}
                    >
                      <div className="col-12 col-md-9 mb-2 mb-md-4 d-flex flex-column flex-md-row align-items-md-center">
                        <ResultThumb
                          ...
                          alt={result.title}
                        />
                        <p className="font-weight-bold mt-4 mt-md-0 mb-0 text-dark ml-md-5">
                          {result.title}
                        </p>
                      </div>
                      <div className="col-12 col-md-3 mb-5 mb-md-4 flex-wrap d-flex align-items-center justify-content-between">
                        ...
                        <div>
                          {result.contentTypes.map((type) => {
                            return (
                              <ContentTypeIcon contentType={type} key={type} />
                            );
                          })}
                        </div>
                      </div>
                ...
                {state.booksResults.slice(0, 2).map((result) => {
                  let thumbnail =
                    result.node.variants[0].image?.originalSrc ||
                    result.node.featuredImage?.originalSrc;

                  return (
                    <Link
                      to={`/book/${result.node.handle}`}
                      className="row gx-5 px-5 px-md-3"
                      key={'link_' + result.node.title}
                    >
                      <div className="col-12 col-md-9 mb-2 mb-md-4 d-flex flex-column flex-md-row align-items-md-center">
                        <ResultThumb
                ...
                {state.coursesResults.slice(0, 2).map((result) => {
                  const {
                    name,
                    slug,
                    course_card_image_url: thumbnail,
                  } = result.node;
                  return (
                    <a
                      href={`https://www.pbcaccess.com/courses/${slug}`}
                      className="row gx-5 px-5 px-md-3"
                      target="_blank"
