...
const Hero = () => {
  ...
  return (
    <HeroSection
      imgSources={{ jpg: HeroImageJPG, webp: HeroImageWebP }}
      style={{ padding: '40px 0' }}
    >
      <div className="container reduced-width my-5">
        <h4
          className={`text-${config.colors.text} font-weight-bold`}
        >{t`Step up. Step forward.`}</h4>
        <h1
          className={`text-${config.colors.text} display-1 font-weight-light mb-3 pb-4 kapra`}
        >{t`Resources to help you grow`}</h1>
        <Link
